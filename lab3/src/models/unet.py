...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # nn.init.normal_(conv1.weight, mean=0, std=math.sqrt(2 / (3 * 3 * out_channels)))
...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, channel_multipliers=[1, 2, 4, 8]):
        ...
        # the last encoder block doesn't have max pooling layer (works like a bottleneck)
        self.blocks.append(ConvBlock(in_channels, out_channels * 2))
    ...
    def forward(self, x):
        ...
        for i, module in enumerate(self.blocks):
            ...
            # the result of last encoder block doesn't need to be stored
            if isinstance(module, ConvBlock) and i != len(self.blocks) - 1:
                x = module(x)
...
class UNet(nn.Module):
    def __init__(
        ...
        # unet consists of an encoder and a decoder
        self.encoder = Encoder(in_channels, base_channels, channel_multipliers)

        # update input channels for decoder
        in_channels = base_channels * channel_multipliers[-1] * 2
        # since we need to upsample, we have to reverse the channel multipliers
        channel_multipliers = channel_multipliers[::-1]
        self.decoder = Decoder(in_channels, base_channels, channel_multipliers)
        ...
        # nn.init.normal_(self.output.weight, mean=0, std=math.sqrt(2 / (1 * 1 * out_channels)))
