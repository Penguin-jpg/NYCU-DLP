...
# loss function (y: ground-truth, y_hat: prediction)
...
class Network:
    ...
    def forward(self, x):
        ...
        # z1 = np.dot(self.input_w, x) + self.input_b  # shape: (4, 1)
        # a1 = self.act(z1)
        # self.values["z1"] = z1
        # self.values["a1"] = a1

        # z2 = np.dot(self.hidden1_w, self.a1) + self.hidden1_b  # shape: (4, 1)
        # a2 = self.act(z2)
        # self.values["z2"] = z2
        # self.values["a2"] = a2

        # z3 = np.dot(self.hidden2_w, self.a2) + self.hidden2_b  # shape: (4, 1)
        # a3 = self.act(z3)
        # self.values["z3"] = z3
        # self.values["a3"] = a3

        # z4 = np.dot(self.output_w, self.a3) + self.output_b  # shape: (1, 1)
        # y = self.act(z4)
        # self.values["z4"] = z4
        # self.values["a4"] = y
    ...
    def backward(self, y, y_hat):
        ...
        # the graident of output layer: dL/d(y_hat) * d(y_hat)/d(z4) * d(z4)/d(w4) = dL/d(w4)
        # derivative of binary cross entropy w.r.t y_hat (dL/d(y_hat)):
        # dL/d(y_hat) = -(y/y_hat - (1-y)/(1-y_hat)) = -(y-y_hat)/(y_hat * (1-y_hat))
        d_L_d_y_hat = -(y / y_hat - (1 - y) / (1 - y_hat))  # y_hat = a4, shape: (1, 1)
        ...
        # the graident of hidden layer 2: dL/d(a4) * d(a4)/d(z4) * d(z4)/d(a3) *
        # d(a3)/d(z3) * d(z3)/d(w3) = dL/d(w3)
        # dL/d(a4) * d(a4)/d(z4) = dL/d(z4)
        d_z4_d_a3 = self.w4  # d(z4)/d(a3)=output_w  (a3 is the "x"), shape: (1, 4)
        ...
        # the graident of hidden layer 1: dL/d(a4) * d(a4)/d(z4) * d(z4)/d(a3) *
        # d(a3)/d(z3) * d(z3)/d(a2) * d(a2)/d(z2) * d(z2)/d(w2) = dL/d(w2)
        # dL/d(a4) * d(a4)/d(z4) * d(z4)/d(a3) * d(a3)/d(z3) = dL/d(z3)
        ...
        # the graident of input layer: dL/d(a4) * d(a4)/d(z4) * d(z4)/d(a3) *
        # d(a3)/d(z3) * d(z3)/d(a2) * d(a2)/d(z2) * d(z2)/d(a1) * d(a1)/d(z1) *
        # d(z1)/d(w1) = dL/d(w1)
        # dL/d(a4) * d(a4)/d(z4) * d(z4)/d(a3) * d(a3)/d(z3) * d(z3)/d(a2) *
        # d(a2)/d(z2) = dL/d(z2)
        ...
        d_L_d_w1 = np.dot(
            np.dot(np.dot(d_L_d_z2.T, d_z2_d_a1), d_a1_d_z1), d_z1_d_w1.T
        )  # shape: (1, 4) dot (4, 4) dot (4, 1) dot (1, 2) = (1, 2)
